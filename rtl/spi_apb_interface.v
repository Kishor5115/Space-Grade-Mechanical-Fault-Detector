...
module spi_apb_interface (
    ...
    always @(posedge clk or negedge sys_rst_n) begin
        if (!sys_rst_n) begin
            ...
            if (sm_data_out_valid) begin
                sample_reg           <= sm_data_out;
                data_ready           <= 1'b1;
                fresh_sample_pending <= 1'b1; // new sample: (re-)arm the forwarder
            end else begin
                if (req_valid_pulse && !req_write && local_addr == ADDR_SAMPLE1) begin
    ...
    always @(posedge clk or negedge sys_rst_n) begin
        if (!sys_rst_n) req_valid_d <= 1'b0;
        else            req_valid_d <= req_valid;
    end
    ...
    always @(posedge clk or negedge sys_rst_n) begin
        if (!sys_rst_n) begin
            ...
            local_done <= 1'b0;
            if (req_valid_pulse) begin
                local_done <= 1'b1;
                case (local_addr)
                    ADDR_STATUS:  local_rdata <= {31'd0, data_ready};
                    ADDR_SAMPLE0: local_rdata <= sample_reg[31:0];
                    ADDR_SAMPLE1: local_rdata <= {16'd0, sample_reg[47:32]};
                    default:      local_rdata <= 32'd0;
                endcase
            end
        end
    end
    ...
    always @(posedge clk or negedge sys_rst_n) begin
        if (!sys_rst_n) begin
            ...
            fwd_req_addr  <= 32'd0;
            fwd_req_wdata <= 32'd0;
        end else if (!tmr_forward_en) begin
